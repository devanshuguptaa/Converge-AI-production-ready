...
async def handle_special_command(
    text: str, user_id: str, channel_id: str, say, thread_ts: str | None
) -> bool:
    """
...
"""
    ...
    if text_lower in ["/reset", "reset", "clear"]:
        ...
        await say(
            "✅ Conversation history cleared! Starting fresh.", thread_ts=thread_ts
        )
        return True

    # Summarize command (only in threads)
    if text_lower in ["summarize", "tldr", "summary"] and thread_ts:
        await say("📝 Summarizing this thread...", thread_ts=thread_ts)
...
async def process_user_message(
    user_id: str, channel_id: str, thread_ts: str | None, text: str, say, client
) -> None:
    """
    ...
    try:
        ...
        try:
            await client.reactions_add(
                channel=channel_id, timestamp=thread_ts or "latest", name="eyes"
            )
        except Exception:
