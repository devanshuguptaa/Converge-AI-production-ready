...
def get_or_create_session(
    user_id: str, channel_id: Optional[str] = None, thread_ts: Optional[str] = None
) -> str:
    """
    ...
    with get_db_session() as db:
        ...
        query = db.query(ConversationSession).filter(
            ConversationSession.user_id == user_id, ConversationSession.is_active
        )
...
def add_message(
    session_id: str, role: str, content: str, msg_metadata: Optional[str] = None
) -> None:
    """
    ...
    with get_db_session() as db:
        message = Message(
            session_id=session_id, role=role, content=content, msg_metadata=msg_metadata
        )
        db.add(message)
        db.commit()
