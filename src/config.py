...
class RAGConfig(BaseSettings):
    ...
    indexer_interval_minutes: int = Field(
        default=60, alias="RAG_INDEXER_INTERVAL_MINUTES"
    )
...
class WhatsAppConfig(BaseSettings):
    ...
    api_url: str = Field(
        default="http://whatsapp-gateway:8080", alias="WHATSAPP_API_URL"
    )
    ...
    instance_name: str = Field(
        default="converge_assistant", alias="WHATSAPP_INSTANCE_NAME"
    )
...
def validate_config() -> tuple[bool, list[str]]:
    ...
    if (
        not config.slack.bot_token
        or config.slack.bot_token == "xoxb-your-bot-token-here"
    ):
        errors.append("SLACK_BOT_TOKEN is not configured")

    if (
        not config.slack.app_token
        or config.slack.app_token == "xapp-your-app-token-here"
    ):
    ...
    if config.memory.enabled:
        if (
            not config.memory.api_key
            or config.memory.api_key == "your-mem0-api-key-here"
        ):
            errors.append(
                "MEM0_API_KEY is not configured (or disable memory with MEMORY_ENABLED=false)"
            )
    ...
    if config.mcp.enabled:
        if not config.mcp.github_token:
            errors.append(
                "GITHUB_PERSONAL_ACCESS_TOKEN is not configured (or disable MCP with MCP_ENABLED=false)"
            )

    # Check WhatsApp configuration (if enabled)
    if config.whatsapp.enabled:
        if (
            not config.whatsapp.api_key
            or config.whatsapp.api_key == "your-whatsapp-api-key-here"
        ):
            errors.append(
                "WHATSAPP_API_KEY is not configured (or disable WhatsApp with WHATSAPP_ENABLED=false)"
            )
