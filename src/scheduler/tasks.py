...
async def execute_scheduled_task(task_id: str):
    ...
    try:
        with get_db_session() as db:
            task = (
                db.query(ScheduledTask).filter(ScheduledTask.task_id == task_id).first()
            )
            ...
            if slack_client:
                await slack_client.chat_postMessage(
                    channel=task.channel_id, text=f"⏰ Reminder: {task.message}"
                )
...
def initialize_scheduler() -> AsyncIOScheduler:
    ...
    try:
        with get_db_session() as db:
            active_tasks = db.query(ScheduledTask).filter(ScheduledTask.is_active).all()
...
@tool
async def set_reminder(user_id: str, channel_id: str, message: str, when: str) -> str:
    """
...
async def schedule_message(
    channel_id: str, message: str, when: str, recurring: bool = False
) -> str:
    """
