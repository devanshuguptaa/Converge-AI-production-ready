...
async def index_channel_messages(channel_id: str, hours_back: int = 24):
    ...
    try:
        ...
        if formatted_messages:
            ...
            logger.info(
                f"Indexed {len(formatted_messages)} messages from channel {channel_id}"
            )
...
async def index_all_channels():
    ...
    try:
        ...
        for channel in channels:
            ...
            await index_channel_messages(
                channel_id, hours_back=config.rag.indexer_interval_minutes // 60
            )
...
def start_indexer():
    ...
    logger.info(
        f"✅ Indexer scheduled to run every {config.rag.indexer_interval_minutes} minutes"
    )
