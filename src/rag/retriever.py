...
async def search_slack_history(
    query: str, channel_id: Optional[str] = None, limit: int = 10
) -> str:
    """
    ...
    try:
        # Search vector store
        results = search_messages(query=query, channel_id=channel_id, limit=limit)
...
if __name__ == "__main__":
    ...
    async def test():
        result = await search_slack_history(query="authentication feature", limit=5)
        print(result)
