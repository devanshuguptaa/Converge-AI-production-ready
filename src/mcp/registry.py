...
async def create_notion_page(database_id: str, title: str, content: str) -> str:
    ...
    return (
        f"⚠️ Notion integration not fully implemented yet. Would create page: '{title}'"
    )
...
async def get_mcp_tools() -> List:
    ...
    try:
        ...
        logger.info(
            f"✅ Added {len(EMAIL_CALENDAR_TOOLS)} email/calendar tools: {[t.name for t in EMAIL_CALENDAR_TOOLS]}"
        )
