...
class EmailTools:
    ...
    def list_recent_emails_tool(self):
        def run(limit: int = 5, query: str = ""):
            self.permission_manager.validate_tool_access(
                "list_recent_emails", MCPScope.GMAIL_READ
            )
    ...
    def get_email_details_tool(self):
        def run(message_id: str):
            self.permission_manager.validate_tool_access(
                "get_email_details", MCPScope.GMAIL_READ
            )
    ...
    def send_email_tool(self):
        def run(to: str, subject: str, body: str):
            self.permission_manager.validate_tool_access(
                "send_email", MCPScope.GMAIL_SEND
            )
    ...
    def create_draft_tool(self):
        def run(to: str, subject: str, body: str):
            self.permission_manager.validate_tool_access(
                "create_draft", MCPScope.GMAIL_SEND
            )  # Using SEND scope for drafts too
    ...
    def summarize_email_thread_tool(self):
        def run(thread_id: str):
            self.permission_manager.validate_tool_access(
                "summarize_email_thread", MCPScope.GMAIL_READ
            )
